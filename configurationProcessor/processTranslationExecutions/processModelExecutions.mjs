...
export async function processModelExecutions (configPath, executionGroup) {
    ...
    while (!allCompleted) {
      ...
      for (const execution of globalPrompts) {
      ...
      for (const { execution, sameLineFactor, } of executionsToProcess) {
        const { name, prefix, language, prompts } = execution;
        const modelDetails = modelMap.get(name);
        ...
        // Use global prompts
        const promptPaths = prompts.map(prompt => join(basePromptsPath, prompt));
