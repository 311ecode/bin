...
/**
 * @typedef {'replace' | 'append' | 'prepend' | 'markConflict'} MergeStrategy
 */

/**
 * @typedef {'diffChars' | 'diffWords' | 'diffWordsWithSpace'} DiffMethod
 */

/**
 * @typedef {Object} MergeVersesOptions
 * @property {boolean} [overwrite] - Whether to overwrite existing verses with new ones. Default: true
 * @property {boolean} [keepExistingOnConflict] - Whether to keep existing verses when there's a conflict and overwrite is false. Default: false
 * @property {boolean} [appendNewVerses] - Whether to append new verses that don't exist in the original content. Default: true
 * @property {MergeStrategy} [mergeStrategy] - The strategy to use when merging conflicting verses. Default: 'replace'
 * @property {string} [delimiter] - The delimiter to use when appending or prepending verses. Default: '||vs||'
 * @property {string} [conflictMarker] - The marker to use when marking conflicts. Default: '<<<CONFLICT>>>'
 * @property {boolean} [showDiff] - Whether to show the diff when marking conflicts. Default: true
 * @property {DiffMethod} [diffMethod] - The diffing method to use when showing diffs. Default: 'diffWords'
 */

/**
 * @typedef {Object.<string, string>} VersesObjectPresentation
 */
 ...
 * @param {VersesObjectPresentation} newVerses - An object containing new verses, where keys are verse numbers and values are verse content.
 * @param {MergeVersesOptions} [options] - Options for merging verses. Any unspecified options will use their default values.
 * @returns {string} The merged content with all verses.
 */

export function mergeVerses(existingContent, newVerses, options = {}) {
  /** @type {MergeVersesOptions} */
