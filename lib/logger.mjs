...
import { isJestTestEnvironment } from './jest-environment-check.mjs';
...
const defaultPossibleMagic = (env.DEBUG_TO_FILE) && 
  ...
  return async (command) => {
    if (!bashProcess && !isJestTestEnvironment()) {
    ...
    !isJestTestEnvironment() && await waitForBashLoaded;
...
export const logger = ({
  ...
  possibleMagic = defaultPossibleMagic,
    ...
    return async (...thingsToLog) => {
      ...
      if (hasOhhmm && !isJestTestEnvironment()) {
